...
def fill_excel(input_path: str, sheet_name: str,
    ...
    """Nur leere Zellen werden neu befüllt; vorhandene Werte bleiben unberührt."""
    # 1) DataFrame lesen
    try:
    ...
    # 2) Scrapen & NUR LEERE ZELLEN füllen
    ...
    for ridx in range(len(df)):
        ...
        for cidx, col in enumerate(df.columns):
            ...
            # *** Nur leere Zellen beschreiben ***
            if not pd.isna(df.iat[ridx, cidx]):
                continue
            ...
            final_val = val_src
            if unit_src and target_unit and unit_src != target_unit:
    ...
    # Nur die NEU geschriebenen Zellen in Excel schreiben (keine bestehenden Werte anfassen)
    for (r, c) in written_cells:
        val = df.iat[(r - 3), (c - 1)]  # zurück auf df-Index (Header=2 Zeilen, 1-basiert nach 0-basiert)
        if pd.isna(val):
            continue
    ...
    wb.save(input_path)
    print(f"[OK] Geschrieben. Backup: {backup_path}. Hervorgehobene (neu befüllte) Zellen: {len(written_cells)}")
    return
