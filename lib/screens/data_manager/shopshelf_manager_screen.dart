// lib/screens/data_manager/shopshelf_manager_screen.dart
...
class ShopshelfManagerScreen extends StatefulWidget {
  const ShopshelfManagerScreen({super.key});

  @override
  State<ShopshelfManagerScreen> createState() => _ShopshelfManagerScreenState();
}

class _ShopshelfManagerScreenState extends State<ShopshelfManagerScreen> {
  ...
  // ---------- Import ----------
  Future<void> _importShopshelf() async {
    _closeSearch();
    setState(() => _importing = true);
    try {
      final affected = await importShopshelfFromCsv();
      _snack('Shopshelf importiert/aktualisiert: $affected Zeilen.');
    } catch (e) {
  ...
  // ---------- CRUD ----------
  Future<void> _addShopshelf() async {
    _closeSearch();
    _nameCtrl.clear();
    _colorCtrl.clear();
    _iconCtrl.clear();
    ...
    final ok = await showDialog<bool>(
      context: context,
      builder: (_) => _ShopshelfEditDialog(
        title: 'Neues Shopshelf anlegen',
    ...
    try {
      await appDb.into(appDb.shopshelf).insert(
            ShopshelfCompanion(
              name: d.Value(name),
      ...
      _snack('Shopshelf "$name" angelegt.');
    } catch (e) {
      _snack('Fehler beim Anlegen: $e');
    }
  }

  Future<void> _editShopshelf(ShopshelfData s) async {
    _closeSearch();
    ...
    try {
      await (appDb.update(appDb.shopshelf)..where((t) => t.id.equals(s.id))).write(
        ShopshelfCompanion(
          name: d.Value(name),
          color: d.Value(color.isEmpty ? null : color),
          icon: d.Value(icon.isEmpty ? null : icon),
      ...
      _snack('Shopshelf #${s.id} aktualisiert.');
    } catch (e) {
      _snack('Fehler beim Speichern: $e');
    }
  }

  Future<void> _deleteShopshelf(ShopshelfData s) async {
    _closeSearch();
    final ok = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        ...
        content: Text('Shopshelf #${s.id} – "${s.name}" wirklich löschen?',
    ...
    try {
      await (appDb.delete(appDb.shopshelf)..where((t) => t.id.equals(s.id))).go();
      _snack('Shopshelf "${s.name}" gelöscht.');
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Shopshelf', style: TextStyle(color: Colors.white)),
        backgroundColor: Colors.black,
      ...
      body: Stack(
        children: [
          StreamBuilder<List<ShopshelfData>>(
            stream: (appDb.select(appDb.shopshelf)
            ...
            builder: (context, snap) {
              ...
              final all = snap.data ?? const <ShopshelfData>[];
              final items = _applyFilter(all);

              if (all.isEmpty) {
                return _emptyView(_importShopshelf);
              }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('Noch keine Shopshelf-Einträge.', style: TextStyle(color: Colors.white70)),
              const SizedBox(height: 12),
              FilledButton.icon(
                onPressed: onImport,
  ...
  Future<bool?> _confirmDelete(ShopshelfData s) => showDialog<bool>(
        context: context,
        builder: (_) => AlertDialog(
          backgroundColor: Colors.black,
          title: const Text('Löschen bestätigen', style: TextStyle(color: Colors.white)),
...
// ---------------------------
// Edit-Dialog (Name, Farbe, Icon mit Vorschau)
// ---------------------------
class _ShopshelfEditDialog extends StatefulWidget {
  final String title;
  ...
  @override
  State<_ShopshelfEditDialog> createState() => _ShopshelfEditDialogState();
}

class _ShopshelfEditDialogState extends State<_ShopshelfEditDialog> {
  @override
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      ...
      content: SingleChildScrollView(
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            _buildTextField('Name', 'z. B. Obst & Gemüse', widget.nameController),
            const SizedBox(height: 12),
            _ColorFieldWithPicker(controller: widget.colorController),
            const SizedBox(height: 12),
            _buildTextField('Icon (Pfad)', 'assets/images/icons/obst_gemuese.png', widget.iconController),
