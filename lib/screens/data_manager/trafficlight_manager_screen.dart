// lib/screens/data_manager/trafficlight_manager_screen.dart
...
class TrafficlightManagerScreen extends StatefulWidget {
  const TrafficlightManagerScreen({super.key});

  @override
  State<TrafficlightManagerScreen> createState() => _TrafficlightManagerScreenState();
}

class _TrafficlightManagerScreenState extends State<TrafficlightManagerScreen> {
  ...
  // ---------- Import ----------
  Future<void> _importTrafficlight() async {
    _closeSearch();
    setState(() => _importing = true);
    try {
      final affected = await importTrafficlightFromCsv(); // assets/data/trafficlight.csv
      _snack('Trafficlight importiert/aktualisiert: $affected Zeilen.');
    } catch (e) {
  ...
  // ---------- CRUD ----------
  Future<void> _addTrafficlight() async {
    _closeSearch();
    _nameCtrl.clear();
    _colorCtrl.clear();
    ...
    final ok = await showDialog<bool>(
      context: context,
      builder: (_) => _TrafficlightEditDialog(
        title: 'Neue Trafficlight-Stufe anlegen',
    ...
    try {
      await appDb.into(appDb.trafficlight).insert(
            TrafficlightCompanion(
              name: d.Value(name),
              color: d.Value(color.isEmpty ? null : color),
            ),
          );
      _snack('Trafficlight "$name" angelegt.');
    } catch (e) {
      _snack('Fehler beim Anlegen: $e');
    }
  }

  Future<void> _editTrafficlight(TrafficlightData s) async {
    _closeSearch();
    ...
    try {
      await (appDb.update(appDb.trafficlight)..where((t) => t.id.equals(s.id))).write(
        TrafficlightCompanion(
          name: d.Value(name),
          color: d.Value(color.isEmpty ? null : color),
        ),
      );
      _snack('Trafficlight #${s.id} aktualisiert.');
    } catch (e) {
      _snack('Fehler beim Speichern: $e');
    }
  }

  Future<void> _deleteTrafficlight(TrafficlightData s) async {
    _closeSearch();
    final ok = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        ...
        content: Text('Trafficlight #${s.id} – "${s.name}" wirklich löschen?',
    ...
    try {
      await (appDb.delete(appDb.trafficlight)..where((t) => t.id.equals(s.id))).go();
      _snack('Trafficlight "${s.name}" gelöscht.');
    } catch (e) {
  ...
  List<TrafficlightData> _applyFilter(List<TrafficlightData> input) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Trafficlight', style: TextStyle(color: Colors.white)),
        backgroundColor: Colors.black,
      ...
      body: Stack(
        children: [
          StreamBuilder<List<TrafficlightData>>(
            stream: (appDb.select(appDb.trafficlight)
            ...
            builder: (context, snap) {
              ...
              final all = snap.data ?? const <TrafficlightData>[];
              final items = _applyFilter(all);

              if (all.isEmpty) {
                return _emptyView(_importTrafficlight);
              }
  ...
  Widget _emptyView(VoidCallback onImport) => Center(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              const Icon(Icons.traffic, size: 48, color: Colors.white70),
              const SizedBox(height: 12),
              const Text('Noch keine Trafficlight-Einträge.',
  ...
  Future<bool?> _confirmDelete(TrafficlightData s) => showDialog<bool>(
        context: context,
        builder: (_) => AlertDialog(
          backgroundColor: Colors.black,
          title: const Text('Löschen bestätigen', style: TextStyle(color: Colors.white)),
...
class _TrafficlightEditDialog extends StatefulWidget {
  final String title;
  final TextEditingController nameController;
  final TextEditingController colorController;
  final String confirmLabel;

  const _TrafficlightEditDialog({
    required this.title,
  ...
  @override
  State<_TrafficlightEditDialog> createState() => _TrafficlightEditDialogState();
}

class _TrafficlightEditDialogState extends State<_TrafficlightEditDialog> {
  @override
  Widget build(BuildContext context) {
    return AlertDialog(
      backgroundColor: Colors.black,
      ...
      content: SingleChildScrollView(
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            _buildTextField('Name', 'z. B. Unverarbeitet / Hochverarbeitet …', widget.nameController),
            const SizedBox(height: 12),
