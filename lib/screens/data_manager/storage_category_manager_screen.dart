// lib/screens/data_manager/storage_category_manager_screen.dart
...
class StorageCategoryManagerScreen extends StatefulWidget {
  const StorageCategoryManagerScreen({super.key});

  @override
  State<StorageCategoryManagerScreen> createState() =>
      _StorageCategoryManagerScreenState();
}

class _StorageCategoryManagerScreenState
    extends State<StorageCategoryManagerScreen> {
  ...
  // ---------- Import ----------
  Future<void> _importStorageCategories() async {
    _closeSearch();
    setState(() => _importing = true);
    try {
      final affected = await importStorageCategoriesFromCsv();
      _snack('Storage Categories importiert/aktualisiert: $affected Zeilen.');
    } catch (e) {
  ...
  // ---------- CRUD ----------
  Future<void> _addStorageCategory() async {
    _closeSearch();
    _nameCtrl.clear();
    _iconCtrl.clear();
    _colorCtrl.clear();
    ...
    final ok = await showDialog<bool>(
      context: context,
      builder: (_) => _StorageCategoryEditDialog(
        title: 'Neue Storage Category anlegen',
        nameController: _nameCtrl,
        iconController: _iconCtrl,
    ...
    try {
      await appDb.into(appDb.storageCategories).insert(
            StorageCategoriesCompanion(
              name: d.Value(name),
              icon: d.Value(icon.isEmpty ? null : icon),
              color: d.Value(color.isEmpty ? null : color),
              description: d.Value(desc.isEmpty ? null : desc),
            ),
          );
      _snack('Storage Category "$name" angelegt.');
    } catch (e) {
      _snack('Fehler beim Anlegen: $e');
    }
  }

  Future<void> _editStorageCategory(StorageCategory s) async {
    _closeSearch();
    _nameCtrl.text = s.name;
    _iconCtrl.text = s.icon ?? '';
    ...
    final ok = await showDialog<bool>(
      context: context,
      builder: (_) => _StorageCategoryEditDialog(
        title: 'Storage Category bearbeiten',
        nameController: _nameCtrl,
        iconController: _iconCtrl,
        colorController: _colorCtrl,
        descController: _descCtrl,
    ...
    try {
      await (appDb.update(appDb.storageCategories)
      ...
      _snack('Storage Category #${s.id} aktualisiert.');
    } catch (e) {
      _snack('Fehler beim Speichern: $e');
    }
  }

  Future<void> _deleteStorageCategory(StorageCategory s) async {
    _closeSearch();
    final ok = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        ...
        title:
            const Text('Löschen bestätigen', style: TextStyle(color: Colors.white)),
        content: Text('Storage Category #${s.id} – "${s.name}" wirklich löschen?',
    ...
    try {
      await (appDb.delete(appDb.storageCategories)
            ..where((t) => t.id.equals(s.id)))
          .go();
      _snack('Storage Category "${s.name}" gelöscht.');
    } catch (e) {
  ...
  List<StorageCategory> _applyFilter(List<StorageCategory> input) {
  ...
  Widget build(BuildContext context) {
    ...
    final overlayBottom = keyboardOpen ? bottomInset : 0.0;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title:
            const Text('Storage Categories', style: TextStyle(color: Colors.white)),
        backgroundColor: Colors.black,
        ...
        actions: [
          if (_importing)
            ...
            IconButton(
              ...
              onPressed: _importStorageCategories,
      ...
      body: Stack(
        children: [
          StreamBuilder<List<StorageCategory>>(
            stream: (appDb.select(appDb.storageCategories)
            ...
            builder: (context, snap) {
              ...
              final all = snap.data ?? const <StorageCategory>[];
              final items = _applyFilter(all);

              if (all.isEmpty) {
                return _emptyView(_importStorageCategories);
              }
              if (items.isEmpty && _query.isNotEmpty) {
                return _noResultsView(_query);
              }

              return ListView.separated(
                padding:
                    const EdgeInsets.fromLTRB(8, 8, 8, searchRowHeight + 8),
                ...
                itemBuilder: (_, i) {
                  final s = items[i];
                  return Dismissible(
                    key: ValueKey('storage-${s.id}'),
                    direction: DismissDirection.horizontal,
                    confirmDismiss: (direction) async {
                      if (direction == DismissDirection.startToEnd) {
                        _editStorageCategory(s);
                        return false;
                      } else {
  ...
  Widget _emptyView(VoidCallback onImport) => Center(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('Noch keine Storage Categories.',
  ...
  Widget _noResultsView(String query) => Center(
        child: Padding(
          padding: const EdgeInsets.all(24),
          child: Column(
            mainAxisSize: MainAxisSize.min,
  ...
  Future<bool?> _confirmDelete(StorageCategory s) => showDialog<bool>(
        context: context,
        builder: (_) => AlertDialog(
          backgroundColor: Colors.black,
...
// =================== Edit-Dialog ===================
class _StorageCategoryEditDialog extends StatefulWidget {
  final String title;
  ...
  const _StorageCategoryEditDialog({
    required this.title,
  ...
  @override
  State<_StorageCategoryEditDialog> createState() => _StorageCategoryEditDialogState();
}

class _StorageCategoryEditDialogState extends State<_StorageCategoryEditDialog> {
  @override
  Widget build(BuildContext context) {
    final iconPath = widget.iconController.text.trim();
    ...
    return AlertDialog(
      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children: [
            _buildTextField('Name', 'z. B. Frischware', widget.nameController),
            const SizedBox(height: 12),
            _buildTextField('Icon-Pfad', 'assets/images/icons/...', widget.iconController),
              ...
              Padding(
                ...
                child: Image.asset(
                  ...
                  errorBuilder: (_, __, ___) =>
                      const Text('Kein gültiges Bild gefunden', style: TextStyle(color: Colors.white54, fontSize: 12)),
                ),
              ),
            const SizedBox(height: 12),
            _buildMultilineField('Beschreibung', 'Kurzer Hinweis zur Kategorie', widget.descController),
            const SizedBox(height: 12),
        ...
        children: [
          ...
          SizedBox(
            ...
            child: FloatingActionButton(
              heroTag: 'addstoragecategorieFab',
