// lib/screens/data_manager/producers_manager_screen.dart
...
class ProducersManagerScreen extends StatefulWidget {
  const ProducersManagerScreen({super.key});

  @override
  State<ProducersManagerScreen> createState() => _ProducersManagerScreenState();
}

class _ProducersManagerScreenState extends State<ProducersManagerScreen> {
  ...
  // ---------- Import ----------
  Future<void> _importProducers() async {
    _closeSearch();
    setState(() => _importing = true);
    try {
      final affected = await importProducersFromCsv();
      _snack('Producers importiert/aktualisiert: $affected Zeilen.');
    } catch (e) {
  ...
  Future<void> _addProducer() async {
    ...
    final ok = await showDialog<bool>(
      context: context,
      builder: (_) => _ProducerEditDialog(
        title: 'Neuen Producer anlegen',
    ...
    try {
      await appDb.into(appDb.producers).insert(
            ProducersCompanion(
              name: d.Value(name),
      ...
      _snack('Producer "$name" angelegt.');
    } catch (e) {
      _snack('Fehler beim Anlegen: $e');
    }
  }

  Future<void> _editProducer(Producer s) async {
    _closeSearch();
    _nameCtrl.text = s.name;
    _colorCtrl.text = s.color ?? '';
    _pictureCtrl.text = s.picture ?? '';

    final ok = await showDialog<bool>(
      context: context,
      builder: (_) => _ProducerEditDialog(
        title: 'Producer bearbeiten',
        nameController: _nameCtrl,
        colorController: _colorCtrl,
        pictureController: _pictureCtrl,
    ...
    try {
      await (appDb.update(appDb.producers)..where((t) => t.id.equals(s.id))).write(
        ProducersCompanion(
          name: d.Value(name),
          color: d.Value(color.isEmpty ? null : color),
          picture: d.Value(picture.isEmpty ? null : picture),
      ...
      _snack('Producer #${s.id} aktualisiert.');
    } catch (e) {
      _snack('Fehler beim Speichern: $e');
    }
  }

  Future<void> _deleteProducer(Producer s) async {
    _closeSearch();
    final ok = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        ...
        content: Text('Producer #${s.id} – "${s.name}" wirklich löschen?',
    ...
    try {
      await (appDb.delete(appDb.producers)..where((t) => t.id.equals(s.id))).go();
      _snack('Producer "${s.name}" gelöscht.');
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Producers', style: TextStyle(color: Colors.white)),
        backgroundColor: Colors.black,
      ...
      body: Stack(
        children: [
          StreamBuilder<List<Producer>>(
            stream: (appDb.select(appDb.producers)
            ...
            builder: (context, snap) {
              ...
              final all = snap.data ?? const <Producer>[];
              final items = _applyFilter(all);

              if (all.isEmpty) return _emptyView(_importProducers);
              if (items.isEmpty && _query.isNotEmpty) return _noResultsView(_query);
  ...
  Widget _emptyView(VoidCallback onImport) => Center(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              const Icon(Icons.factory, size: 48, color: Colors.white70),
              const SizedBox(height: 12),
              const Text('Noch keine Producer-Einträge.', style: TextStyle(color: Colors.white70)),
              const SizedBox(height: 12),
              FilledButton.icon(
                onPressed: onImport,
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text('Keine Treffer für „$query“', style: const TextStyle(color: Colors.white70)),
            ],
          ),
        ),
      );

  Future<bool?> _confirmDelete(Producer s) => showDialog<bool>(
        context: context,
        builder: (_) => AlertDialog(
          backgroundColor: Colors.black,
          title: const Text('Löschen bestätigen', style: TextStyle(color: Colors.white)),
  ...
  Widget _swipeBg({required Alignment alignment, required Color color, required IconData icon}) =>
      Container(
        color: color,
        alignment: alignment,
...
// Edit-Dialog
// ---------------------------
class _ProducerEditDialog extends StatefulWidget {
  final String title;
  ...
  @override
  State<_ProducerEditDialog> createState() => _ProducerEditDialogState();
}

class _ProducerEditDialogState extends State<_ProducerEditDialog> {
  @override
  Widget build(BuildContext context) {
    final picturePath = widget.pictureController.text.trim();
    return AlertDialog(
      backgroundColor: Colors.black,
      ...
      content: SingleChildScrollView(
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            _buildTextField('Name', 'z. B. Rapunzel', widget.nameController),
            const SizedBox(height: 12),
            _ColorFieldWithPicker(controller: widget.colorController),
            const SizedBox(height: 12),
            _buildTextField('Bild (Pfad)', 'assets/images/shop/rapunzel.webp', widget.pictureController),
