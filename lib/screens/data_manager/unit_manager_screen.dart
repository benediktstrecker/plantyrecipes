...
class _UnitManagerScreenState extends State<UnitManagerScreen> {
  ...
  Future<void> _importUnits() async {
    ...
    try {
      final affected = await importUnitsFromCsv();
      _snack('Units importiert/aktualisiert: $affected Zeilen.');
    } catch (e) {
  ...
  Future<void> _addUnit() async {
    ...
    try {
      ...
      _snack('Einheit "$label" angelegt.');
    } catch (e) {
      _snack('Fehler beim Anlegen: $e');
    }
  }

  Future<void> _editUnit(Unit u) async {
    _closeSearch();
    ...
    try {
      await (appDb.update(appDb.units)..where((t) => t.id.equals(u.id))).write(
        UnitsCompanion(
          code: d.Value(code),
          label: d.Value(label),
          dimension: d.Value(dimension),
      ...
      _snack('Einheit #${u.id} aktualisiert.');
    } catch (e) {
      _snack('Fehler beim Speichern: $e');
    }
  }

  Future<void> _deleteUnit(Unit u) async {
    _closeSearch();
    final ok = await showDialog<bool>(
      context: context,
  ...
  List<Unit> _applyFilter(List<Unit> input) {
      ...
      appBar: AppBar(
        ...
        actions: [
          if (_importing)
            ...
            IconButton(
              tooltip: 'Units aus CSV importieren',
        ...
        children: [
          ...
          NotificationListener<ScrollNotification>(
            ...
            child: StreamBuilder<List<Unit>>(
              stream: (appDb.select(appDb.units)
              ...
              builder: (context, snap) {
                ...
                final allUnits = snap.data ?? const <Unit>[];
                final units = _applyFilter(allUnits);
                  ...
                  return Center(
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          const Icon(Icons.straighten, size: 48, color: Colors.white70),
                          const SizedBox(height: 12),
                          const Text('Noch keine Einheiten eingetragen.',
                              style: TextStyle(color: Colors.white70)),
                          const SizedBox(height: 12),
                          FilledButton.icon(
                            onPressed: _importUnits,
                            icon: const Icon(Icons.file_download),
                            label: const Text('Units aus CSV importieren'),
                          ),
                        ],
                      ),
                    ),
                ...
                if (units.isEmpty && _query.isNotEmpty) {
                  return Center(
                    child: Padding(
                      padding: const EdgeInsets.all(24),
                      child: Column(
    ...
    return AlertDialog(
      ...
      content: SingleChildScrollView(
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            _buildField('Code', 'z. B. g', codeController),
            const SizedBox(height: 12),
