// lib/screens/data_manager/markets_manager_screen.dart
...
class MarketsManagerScreen extends StatefulWidget {
  const MarketsManagerScreen({super.key});

  @override
  State<MarketsManagerScreen> createState() => _MarketsManagerScreenState();
}

class _MarketsManagerScreenState extends State<MarketsManagerScreen> {
  ...
  Future<void> _importMarkets() async {
    ...
    try {
      final affected = await importMarketsFromCsv();
      _snack('Markets importiert/aktualisiert: $affected Zeilen.');
    } catch (e) {
  ...
  Future<void> _addMarket() async {
    ...
    try {
      await appDb.into(appDb.markets).insert(
            MarketsCompanion(
              name: d.Value(name),
      ...
      _snack('Market "$name" angelegt.');
    } catch (e) {
      _snack('Fehler beim Anlegen: $e');
    }
  }

  Future<void> _editMarket(Market s) async {
    _closeSearch();
    ...
    try {
      await (appDb.update(appDb.markets)..where((t) => t.id.equals(s.id))).write(
        MarketsCompanion(
          name: d.Value(name),
          color: d.Value(color.isEmpty ? null : color),
          picture: d.Value(picture.isEmpty ? null : picture),
      ...
      _snack('Market #${s.id} aktualisiert.');
    } catch (e) {
      _snack('Fehler beim Speichern: $e');
    }
  }

  Future<void> _deleteMarket(Market s) async {
    _closeSearch();
    final ok = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        ...
        content: Text('Market #${s.id} – "${s.name}" wirklich löschen?',
    ...
    try {
      await (appDb.delete(appDb.markets)..where((t) => t.id.equals(s.id))).go();
      _snack('Market "${s.name}" gelöscht.');
    } catch (e) {
  ...
  List<Market> _applyFilter(List<Market> input) {
      ...
      body: Stack(
        children: [
          StreamBuilder<List<Market>>(
            stream: (appDb.select(appDb.markets)
            ...
            builder: (context, snap) {
              ...
              final all = snap.data ?? const <Market>[];
              final items = _applyFilter(all);

              if (all.isEmpty) return _emptyView(_importMarkets);
  ...
  Widget _emptyView(VoidCallback onImport) => Center(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              const Icon(Icons.storefront, size: 48, color: Colors.white70),
              const SizedBox(height: 12),
              const Text('Noch keine Market-Einträge.', style: TextStyle(color: Colors.white70)),
              const SizedBox(height: 12),
              FilledButton.icon(
                onPressed: onImport,
  ...
  Future<bool?> _confirmDelete(Market s) => showDialog<bool>(
        context: context,
        builder: (_) => AlertDialog(
          backgroundColor: Colors.black,
          title: const Text('Löschen bestätigen', style: TextStyle(color: Colors.white)),
...
// ---------------------------
// Edit-Dialog (Name, Farbe, Bild mit Vorschau)
// ---------------------------
class _MarketEditDialog extends StatefulWidget {
  final String title;
  ...
  @override
  State<_MarketEditDialog> createState() => _MarketEditDialogState();
}

class _MarketEditDialogState extends State<_MarketEditDialog> {
  @override
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      ...
      content: SingleChildScrollView(
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            _buildTextField('Name', 'z. B. Rewe', widget.nameController),
            const SizedBox(height: 12),
            _ColorFieldWithPicker(controller: widget.colorController),
            const SizedBox(height: 12),
            _buildTextField('Bild (Pfad)', 'assets/images/shop/rewe.webp', widget.pictureController),
