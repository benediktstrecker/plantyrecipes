// lib/screens/data_manager/tag_categorie_manager_screen.dart
...
class TagCategorieManagerScreen extends StatefulWidget {
  const TagCategorieManagerScreen({super.key});

  @override
  State<TagCategorieManagerScreen> createState() => _TagCategorieManagerScreenState();
}

class _TagCategorieManagerScreenState extends State<TagCategorieManagerScreen> {
  // Edit-Felder
  ...
  // ---------- Import ----------
  Future<void> _importTagCategories() async {
    _closeSearch();
    setState(() => _importing = true);
    try {
      final affected = await importTagCategoriesFromCsv(); // assets/data/tag_categorie.csv
      _snack('Tag-Kategorien importiert/aktualisiert: $affected Zeilen.');
    } catch (e) {
  ...
  // ---------- CRUD ----------
  Future<void> _addTagCategory() async {
    _closeSearch();
    _nameCtrl.clear();
    _colorCtrl.clear();
    ...
    final ok = await showDialog<bool>(
      context: context,
      builder: (_) => _TagCategorieEditDialog(
        title: 'Neue Tag-Kategorie anlegen',
    ...
    try {
      await appDb.into(appDb.tagCategories).insert(
            TagCategoriesCompanion(
              name: d.Value(name),
              color: d.Value(color.isEmpty ? null : color),
            ),
          );
      _snack('Tag-Kategorie "$name" angelegt.');
    } catch (e) {
      _snack('Fehler beim Anlegen: $e');
    }
  }

  Future<void> _editTagCategory(TagCategory t) async {
    _closeSearch();
    ...
    final ok = await showDialog<bool>(
      context: context,
      builder: (_) => _TagCategorieEditDialog(
        title: 'Tag-Kategorie bearbeiten',
        nameController: _nameCtrl,
        colorController: _colorCtrl,
    ...
    try {
      await (appDb.update(appDb.tagCategories)..where((c) => c.id.equals(t.id))).write(
        TagCategoriesCompanion(
          name: d.Value(name),
          color: d.Value(color.isEmpty ? null : color),
        ),
      );
      _snack('Tag-Kategorie #${t.id} aktualisiert.');
    } catch (e) {
      _snack('Fehler beim Speichern: $e');
    }
  }

  Future<void> _deleteTagCategory(TagCategory t) async {
    _closeSearch();
    final ok = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        ...
        content: Text('Tag-Kategorie #${t.id} – "${t.name}" wirklich löschen?',
    ...
    try {
      await (appDb.delete(appDb.tagCategories)..where((c) => c.id.equals(t.id))).go();
      _snack('Tag-Kategorie "${t.name}" gelöscht.');
    } catch (e) {
  ...
  List<TagCategory> _applyFilter(List<TagCategory> input) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Tag-Kategorien', style: TextStyle(color: Colors.white)),
        backgroundColor: Colors.black,
        ...
        actions: [
          if (_importing)
            ...
            IconButton(
              ...
              onPressed: _importTagCategories,
              icon: const Icon(Icons.file_download, color: Colors.white),
            ),
        ],
      ),

      // exakt wie Referenz: feste NavigationBar unten
      ...
      body: Stack(
        children: [
          ...
          NotificationListener<ScrollNotification>(
            ...
            child: StreamBuilder<List<TagCategory>>(
              stream: (appDb.select(appDb.tagCategories)
              ...
              builder: (context, snap) {
                ...
                final all = snap.data ?? const <TagCategory>[];
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const Text('Noch keine Tag-Kategorien.',
                        ...
                        if (direction == DismissDirection.startToEnd) {
                          ...
                          final ok = await showDialog<bool>(
                            ...
                            builder: (_) => AlertDialog(
                              ...
                              content: Text(
                                  'Tag-Kategorie #${t.id} – "${t.name}" wirklich löschen?',
                                  style: const TextStyle(color: Colors.white70)),
                              actions: [
                                TextButton(
                                  onPressed: () => Navigator.pop(context, false),
                          ...
                          if (ok == true) {
                            await (appDb.delete(appDb.tagCategories)
...
class _TagCategorieEditDialog extends StatefulWidget {
  final String title;
  final TextEditingController nameController;
  final TextEditingController colorController;
  final String confirmLabel;

  const _TagCategorieEditDialog({
    required this.title,
  ...
  @override
  State<_TagCategorieEditDialog> createState() => _TagCategorieEditDialogState();
}

class _TagCategorieEditDialogState extends State<_TagCategorieEditDialog> {
  @override
  Widget build(BuildContext context) {
    return AlertDialog(
      backgroundColor: Colors.black,
      ...
      content: SingleChildScrollView(
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            _buildTextField('Name', 'z. B. Ernährungsform, Küchenstil …', widget.nameController),
            const SizedBox(height: 12),
